...
class operator:
    ...
    def __repr__(self):
        return f"operator({repr(self.value)[7: - 1]})".replace(", dtype=object", "")

    def __str__(self):
        return f"operator({str(self.value)})".replace(", dtype=object", "")
...
class QFT(operator):
    ...
    def __init__(self, degree):
        ...
        w = complex(e**d)
        value = [
            [(w ** (i * j)) for i in range(2**degree)] for j in range(2**degree)
        ]
...
class ising(operator):
    def __init__(self, phi):
        value = [
            [one, zero, zero, -I * e ** (I * phi)],
            [zero, one, -I, zero],
            [zero, -I, one, zero],
            [-I * e ** (I * phi), zero, zero, one],
        ]
